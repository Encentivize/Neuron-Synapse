...
function getClientToken(inputs, callback) {
    ...
    var req = client.post(url, args, function (data, response) {
        ...
        try {
            ...
            return callback(new Error("Error converting token string to JSON object, token string was : " + tokenString));
        }
        if(!token){
            return callback(new Error("Token string resulted in a blank object, token string was : " + tokenString));
        }
